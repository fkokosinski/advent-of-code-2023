...
hand_type(Hand, Type) :-
	...
	% remove jokers
	string_codes("J", [Joker]),
	delete(Codes, Joker, CodesNoJoker),

	% count removed jokers
	length(CodesNoJoker, LenNoJoker),
	RemovedJokers is 5 - LenNoJoker,

	% find which card to duplicate
	(
		RemovedJokers < 5 ->
		chars_stat(CodesNoJoker, StatNoJoker),
		findall([Stat, Card], (Stat = StatNoJoker.get(Card)), CardsCount),
		sort(0, @>=, CardsCount, [[_, ToDuplicate]|_])
	;
		string_codes("A", [ToDuplicate])
	),

	% generate a hand of cards with jokers substituted
	length(NewCodes, RemovedJokers),
	maplist(=(ToDuplicate), NewCodes),
	append(NewCodes, CodesNoJoker, NewCards),

	chars_stat(NewCards, Stat),
	hand_type(NewCards, Stat, Type).

is_stronger('=', [], []).
is_stronger(Delta, [H1|T1], [H2|T2]) :-
	string_codes("J23456789TQKA", LabelOrder),
